    ...
    for user_id, data in attendance_data.items():
        ...
        for date, entry_data in dates.items():
            ...
            for row in range(max_rows):
                ...
                pdf.ln()
                cursor.execute("INSERT INTO my_table (id, name, date, came_time, left_time) VALUES (%s, %s, %s, %s, %s)",
                               (id_counter, user_name if row == 0 else '', date if row == 0 else '',
                                came_times[row] if row < len(came_times) else '',
                                left_times[row] if row < len(left_times) else ''))
